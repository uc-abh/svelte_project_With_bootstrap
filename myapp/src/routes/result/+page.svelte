<script>
	...
	$: for (let i = 0; i < $questiondata_store.length; i++) {
		...
		if ($choosingans_store[i]) {
			for (let j = 0; j < 4; j++) {
				if (
					JSON.parse($questiondata_store[i].content_text).answers[j].answer == $choosingans_store[i]
				) {
					correct_index = j;
				}
			}
...
<div class="mycontainer">
	...
	<div class="container d-flex justify-content-center mt-4 flex-column align-items-center">
		<div class="d-flex" style="width: 800px;">
			<div class="d-flex flex-column align-items-center btn  border rounded w-50">
				<p class="text-info">{percentage} %</p>
				<h6 class="text-info">Result</h6>
			</div>
			<div class="d-flex flex-column align-items-center btn border rounded w-50  ms-3">
			...
			<tbody>
				{#each $questiondata_store as data, i}
					<tr>
						...
						<td>
							<a href="review?qno={i + 1}" class="w-100 text-decoration-none text-white {i}">
								{data.snippet}
							</a>
						</td>
						<td class="d-flex">
